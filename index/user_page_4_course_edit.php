...
//   $course_data_status = strip_tags($_POST['course_data_status']);
  $course_data_status = isset($_POST['course_data_status']) ? $_POST['course_data_status'] : false; 
 ...
 if($total_data <=0){
  ...
  $sql_process->update($table2,$fields2,$Where2); 
   ...
   $exception->getMessage();  // Should be handled with a proper error message.
    ...
    echo "<script>";
    echo "location.href = '?option=course-data-edit&coe=$course_code&success'";
    echo "</script>";
}
...
$stmt2->execute(array(":course_code_param"=>$course_code_get,":school_id_param"=>$school_id));
...
<section class="content-header">
  ...
  <li><a href="?option=UAJDPLCUTKIIHA">ข้อมูลหลักสูตร </a></li>
...
<input type="hidden" name="course_code" id="course_code" value="<?=$_GET["coe"]?>"/>
<div class="row">
              <div class="col-xs-6">
            ...
            //  echo "<option value='$rowB->vehicle_type_id'>$rowB->vehicle_type_name</option>";
             ...
             echo ">$rowB->vehicle_type_name</option>\n";
                ...
                <div class="col-xs-6">
                  ...
                  <input OnKeyPress="return chkNumber(this)"  autocomplete="off" maxlength="10" type="text" onDrag="return false;" onDrop="return false;" onPaste="return false;" onMouseDown="noRightClick(event);" onKeyDown="return noPaste(event);"  class="form-control" name="cp_price" id="cp_price" value="<?=$dataRow2['cp_price'];?>"  >
                </div>
            
                <?php if($dataRow['school_id']!='0') {?>           
                <div class="col-xs-6">
