...
foreach($list as $id) {
	try {
        $query = $sql_process->runQuery("UPDATE tbl_program_permission SET page_number_prog_id = :item_order 
		 WHERE  grp_id =:grp_id_param AND prog_id=:id AND user_group_id=:user_group_id_param AND school_id=:school_id_param");
        ...
        $query->bindParam(':grp_id_param', $grp_id, PDO::PARAM_INT);
		$query->execute();
