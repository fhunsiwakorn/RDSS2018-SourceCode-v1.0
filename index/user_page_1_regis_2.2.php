...
tbl_register_temp.start_time,
tbl_register_temp.end_time,
...
$vehicle_type_id_x=$dataRow["vehicle_type_id"];
$start_time=$dataRow["start_time"]  ;
$end_time =$dataRow["end_time"] ;
...
echo ">$yt</option>\n";
  ...
  ?>
    ...
    <div class="col-xs-12">
    <b>
    หลักสูตร : <?=$dataRow["course_data_name"]?> ช่วงเวลา :  <?=$dataRow["start_time"]?> -  <?=$dataRow["end_time"]?>  
    </b>
    </div>
    <div class="col-xs-12">
...
//  echo "<a href='?option=regis-step-2&sccode=$rst_temp_code&setdate=$sday'>";
 echo $day_format;
//  echo "</a>";
 echo "</span>";
 echo "<br>";
...
TIME(tbl_register_main_class_schedule.rmcs_start_date) NOT BETWEEN '$start_time'  AND '$end_time') AND

(tbl_register_main_class_schedule.vehicle_type_id = :vehicle_type_id_param AND
tbl_register_main_class_schedule.school_id = :school_id_param AND
tbl_register_main_class_schedule.type_study_id = '2' AND
TIME(tbl_register_main_class_schedule.rmcs_date_end) NOT BETWEEN '$start_time'  AND '$end_time')  
   ...
   ");
   // $qgqwr->execute();
   $qgqwq->execute(array(":school_id_param"=>$school_id,":vehicle_type_id_param"=>$vehicle_type_id_x));
...
<table class="table">
 ...
 if(isset($_GET['Vr'])){
  ...
  $state="MONTH(tbl_register_main_class_schedule.rmcs_start_date) = '$month_explode' AND YEAR(tbl_register_main_class_schedule.rmcs_start_date) = '$year_explode'";
  $group="GROUP BY DATE(tbl_register_main_class_schedule.rmcs_start_date),tbl_register_main_class_schedule.trainer_id";
}
$num_gen2='0';
...
(tbl_register_main_class_schedule.vehicle_type_id = :vehicle_type_id_param AND
tbl_register_main_class_schedule.school_id = :school_id_param AND
tbl_register_main_class_schedule.trainer_id = tbl_trainer_data.trainer_id AND
tbl_register_main_class_schedule.type_study_id = '2' AND
TIME(tbl_register_main_class_schedule.rmcs_start_date) NOT BETWEEN '$start_time'  AND '$end_time' AND
 $state) AND
(tbl_register_main_class_schedule.vehicle_type_id = :vehicle_type_id_param AND
...
tbl_register_main_class_schedule.trainer_id = tbl_trainer_data.trainer_id AND
tbl_register_main_class_schedule.type_study_id = '2' AND
TIME(tbl_register_main_class_schedule.rmcs_date_end) NOT BETWEEN '$start_time'  AND '$end_time' AND
 $state)  

 $group

ORDER BY
tbl_register_main_class_schedule.rmcs_start_date ASC
");
// $qgqwr->execute();
$qgqwr->execute(array(":school_id_param"=>$school_id,":vehicle_type_id_param"=>$vehicle_type_id_x));
while($rowTea= $qgqwr->fetch(PDO::FETCH_OBJ)) {
$num_gen2++;

$arraydate2 = explode(" ",$rowTea->rmcs_start_date);
$senddate= $arraydate2[0];//วันที่ห
...
?>
                  ...
                  <td><?=$num_gen2?></td>
                  <td>
                  <a href='?option=<?=$_GET["option"]?>&setdate=<?=$senddate?>&t=<?=$rowTea->trainer_id?>&sccode=<?=$rst_temp_code?>'>  <?=$rowTea->trainer_firstname_th?> <?=$rowTea->trainer_lastname_th?>  </a>
                 <!-- <br> <?php echo DateTimetotime($rowTea->rmcs_start_date)?> ถึง <?php echo DateTimetotime($rowTea->rmcs_date_end)?>  -->
                  </td>
                  <td>
                  <?php echo DateThai($rowTea->rmcs_start_date)?> 
                  </td>
                </tr>
<?php } ?>
              </table>
