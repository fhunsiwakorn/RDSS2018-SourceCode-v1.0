...
// $register_code_get = $_GET['sccode'];	
$register_code_get = isset($_GET['registerCode']) ? $_GET['registerCode'] : NULL;
$stmt = $sql_process->runQuery("SELECT student_id,student_ID_card,student_firstname_th,student_lastname_th FROM tbl_student_data
WHERE register_code=:register_code_param 
");
$stmt->execute(array(":register_code_param"=>$register_code_get));
$dataRow=$stmt->fetch(PDO::FETCH_ASSOC);
$student_id=$dataRow['student_id'];
$student_ID_card=$dataRow['student_ID_card'];
$student_firstname_th=$dataRow['student_firstname_th'];
$student_lastname_th=$dataRow['student_lastname_th'];
...
$stmt_1 = $sql_process->runQuery("SELECT vat_status,vat_percent FROM tbl_school WHERE school_id='$school_id'");
$stmt_1->execute();
$dataRow_1=$stmt_1->fetch(PDO::FETCH_ASSOC);
if($register_code_get ==NULL){
  $sqlSt=NULL;
}else{
  $sqlSt="AND tbl_student_data.student_ID_card ='$student_ID_card'";
}
?>
...
limit $start,$rows
");
// $qg->execute(); $student_ID_card
$qg->execute(array(":school_id_param"=>$school_id));
while($rowGen= $qg->fetch(PDO::FETCH_OBJ)) {
$num_gen++;
// $dataR=$sql_process->QueryField1("SELECT  MAX(rp_id) as rp_id, rp_total_pay FROM tbl_register_pay   WHERE register_code='$rowGen->register_code' AND school_id='$school_id'");
