...
if(isset($_POST['btn-submit-add']))
{
  ...
  $user_list->register_user($user_name,$user_password,$user_prefix,$user_firstname,$user_lastname,$user_email,$school_id,$branch_id,$user_group_id,$user_crt,$user_flag,$user_status);
    echo "<script>";
    echo "location.href = '?option=4UKiyHwBDd9&success'";
    echo "</script>";
  }
}
...
<section class="content-header">
  <h1>
เจ้าหน้าที่ผู้ใช้งาน
    <small>จัดการข้อมูลเจ้าหน้าที่ผู้ใช้งาน</small>
  </h1>
  <ol class="breadcrumb">
  <li><a href="?option=main"><i class="fa fa-dashboard"></i> หน้าหลัก</a></li>
  <!-- <li ><a href="?option=4UKiyHwBDd9">ทะเบียนเจ้าหน้าที่</a></li> -->
  <li class="active">เจ้าหน้าที่ผู้ใช้งาน</li>
  </ol>
</section>
<!-- Main content -->
<section class="content">
  ...
  <div class="box">
    <div class="box-header with-border">
      <h3 class="box-title">เจ้าหน้าที่ผู้ใช้งาน</h3><br>
...
tbl_branch.branch_status='1' AND
tbl_branch.school_id=:school_id_param
ORDER BY
tbl_branch.branch_id ASC");
$qa8->execute(array(":school_id_param"=>$school_id));
while($rowI= $qa8->fetch(PDO::FETCH_OBJ)) {
echo "<option value='$rowI->branch_id'>$rowI->branch_name</option>";
}
?>             
</select>
</div>              
...
tbl_user.school_id =:school_id_param
ORDER BY
...
<span class="cr"><i class="cr-icon glyphicon glyphicon-ok"></i></span>
 ...
 <!-- <?php echo  sprintf("%02d", $num_gen); ?>.) -->
        ...
        <button type="button" class="btn btn-warning"  onclick="window.location.href='?option=user-edit&auttid=<?=$rowGen->user_id?>'" >
        แก้ไข
