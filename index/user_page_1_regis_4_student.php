...
$stmtDATE = $sql_process->runQuery("SELECT
...
$stmtDATE->execute(array(":rst_temp_code_param"=>$rst_temp_code_get,":school_id_param"=>$school_id,":ip_pc_param"=>$ipaddress));
$dateRowA=$stmtDATE->fetch(PDO::FETCH_ASSOC);
$rst_temp_date_calendar=$dateRowA["rst_temp_date_calendar"];
$rst_temp_code=$dateRowA["rst_temp_code"]; 
////นักเรียน
$stmt = $sql_process->runQuery("SELECT * FROM tbl_register_temp_student WHERE rst_temp_code=:rst_temp_code_param");
$stmt->execute(array(":rst_temp_code_param"=>$rst_temp_code_get));
$dataRow=$stmt->fetch(PDO::FETCH_ASSOC);
//โค้ดอ้างอิงการรับสมัคร
    ...
    require_once('user_page_1_regis_4_student_form_logChk.php'); 
?>
...
<section class="content">
  ...
  <div class="box">
    <div class="box-header with-border">
      <h3 class="box-title">สมัครเรียน ขั้นตอนที่ 4</h3>
...
<input type="hidden" name="rst_temp_date_calendar" value="<?=$rst_temp_date_calendar?>"/>
...
<select class="form-control" style="width: 100%;" name="title_name_eng" id="title_name_eng"  >
<option>-- </option>
<?php
...
<input type="text" class="form-control" value="<?=$dataRow['student_firstnam_eng']?>" name="student_firstnam_eng" id="student_firstnam_eng"  placeholder="Firstname">
</div>
<div class="col-xs-4">
<label >Lastname</label>        
<input type="text" class="form-control" value="<?=$dataRow['student_lastname_eng']?>" name="student_lastname_eng" id="student_lastname_eng"  placeholder="Lastname">
</div>
...
<?php
if($dataRow["student_birthday"]=="0000-00-00"){
  $birthday=NULL;
}else{
$birthday=DatetoDMY($dataRow["student_birthday"]);
}

?>
<input type="text"class="form-control pull-right" value="<?=$birthday?>"  name="student_birthday" required  data-inputmask="'alias': 'dd/mm/yyyy'" data-mask  placeholder="วันเกิด" >
