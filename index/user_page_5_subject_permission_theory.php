...
require_once('user_page_5_subject_permission_theory_logChk.php');
...
ข้อมูลหลักสูตร
  ...
  <li><a href="?option=<?=$_GET["option"]?>">ข้อมูลหลักสูตร</a></li>
...
tbl_subject_data.vehicle_type_id=:vehicle_type_param AND
...
$qx->execute(array(":school_id_param"=>$school_id,":vehicle_type_param"=>$vehicle_type_id));
...
<form method="post" id="form_add"  name="form_add" runat="server">
<input type="hidden" name="option" value="<?=$_GET["option"]?>">
...
<input type="text" name="st_date[]" value="<?=$st_date?>"   style="width: 300px" class="form-control pull-right" data-inputmask="'alias': 'dd/mm/yyyy'" id="datepicker<?=$num_gen?>" required placeholder="คลิ๊กเพื่อแสดงปฏิทิน" disabled>
