...
echo "<script>";
echo "location.href = '?option=MS6KUDQC770FJQF&success'";
echo "</script>";
...
<section class="content">
    ...
    <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <div class="col-xs-6">
                  ...
                  <input type="text" name="esd_date"  class="form-control pull-right" data-inputmask="'alias': 'dd/mm/yyyy'" id="datepickerX" required readonly placeholder="คลิ๊กเพื่อแสดงปฏิทิน">
  ...
  echo "<option value='$rowA->type_study_id'>$rowA->type_study_name</option>";
                 ...
                 ?>           
                  ...
                  <input type="time" class="form-control" name="esd_start_time" id="esd_start_time"  required placeholder="เวลาเริ่ม">
                </div> 
                <div class="col-xs-6">
                  <label for="exampleInputEmail1">เวลาสิ้นสุด</label> <label style="color:red;">*</label>
                
                  <input type="time" class="form-control" name="esd_end_time" id="esd_end_time"  required placeholder="เวลาสิ้นสุด">
                </div>  
...
$qre= $sql_process->runQuery(
    "SELECT
     ...
     ORDER BY
     trainer_id");
 $qre->execute(array(":school_id_param"=>$school_id));
    while($rs= $qre->fetch(PDO::FETCH_OBJ)) {  
    ...
    <div class="col-xs-4">
      <label>ประเภทหลักสูตร  </label> <label style="color:red;">*</label>
                   <select class="form-control" name="vehicle_type" id="vehicle_type_id" required onchange="submit();">
...
tbl_exam_schedule.vehicle_type_id =:vehicle_type_param AND
...
");
// $qg->execute();
$qg->execute(array(":school_id_param"=>$school_id,":vehicle_type_param"=>$vehicle_type_id));
                    ...
                    <script>
                        function move<?=$num_gen?>() {
                          ...
                          function(isConfirm){
                            ...
                            location.href = '?option=<?=$_GET["option"]?>&pidcode=<?=$rowGen->esd_code?>';
                            } else {
