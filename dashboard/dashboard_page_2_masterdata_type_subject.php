...
if(isset($_POST['btn-submit-add']))
{
  ...
  $type_subject_name = strip_tags($_POST['type_subject_name']);
  $type_subject_status = strip_tags($_POST['type_subject_status']);
  $crt_by=$_SESSION['userSession']; 
  $crt_date=date("Y-m-d H:i:s");
  $studey_type_list->add_subject_type($type_subject_name,$crt_by,$crt_date,$type_subject_status);
    echo "<script>";
    echo "location.href = '?option=subject-type&success'";
    echo "</script>";
}
if(isset($_POST['btn-submit-edit']))
{
    $type_subject_id = strip_tags($_POST['type_subject_id']);
    $type_subject_name = strip_tags($_POST['type_subject_name_edit']);
    $type_subject_status = strip_tags($_POST['type_subject_status_edit']);
    $upd_by=$_SESSION['userSession'];
    $studey_type_list->edit_subject_type($type_subject_id,$type_subject_name,$upd_by,$type_subject_status);
    echo "<script>";
    echo "location.href = '?option=subject-type&success'";
    echo "</script>";
} 
if(isset($_POST['DELtype_subject_id'])) {
  $count=count($_POST['DELtype_subject_id']);
  for($i=0;$i<$count;$i++){
   $type_subject_id = $_POST['DELtype_subject_id'];
   $studey_type_list->delete_subject_type($type_subject_id[$i]);
   echo "<script>";
   echo "location.href = '?option=subject-type&success'";
   echo "</script>";
    }
...
<section class="content-header">
  ...
  <li class="active">ประเภทรายวิชา</li>
  </ol>
</section>
<!-- Main content -->
<section class="content">
  ...
  <div class="box">
    <div class="box-header with-border">
      <h3 class="box-title">ประเภทรายวิชา</h3><br>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title">ประเภทรายวิชา</h4>
              </div>
              <form  method="post" name="form_add"id="form_add">
              <div class="modal-body">          
              <div class="form-group">
                  <label for="exampleInputEmail1">ประเภทรายวิชา</label>
                
                  <input type="text" class="form-control" name="type_subject_name" id="type_subject_name" required placeholder="กรอกประเภทรายวิชา">
                </div>    
                <div class="form-group">

                   <label>สถานะการใช้งาน</label>
                   <select name="type_subject_status" id="type_subject_status"  class="form-control">
...
tbl_master_type_subject.crt_by,
tbl_master_type_subject.crt_date,
tbl_master_type_subject.upd_by,
tbl_master_type_subject.upd_date,
tbl_master_type_subject.type_subject_status,
...
tbl_user
WHERE
tbl_master_type_subject.crt_by = tbl_user.user_id  AND
...
<div class="checkbox">
<label style="font-size: 1em">
<input type="checkbox"  name="DELtype_subject_id[]" id="pNUM"  value="<?=$rowGen->type_subject_id?>">
<span class="cr"><i class="cr-icon glyphicon glyphicon-ok"></i></span>
</label>
        ...
        <div class="modal fade" id="modal-edit-<?=$num_gen?>">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <input type="hidden" class="form-control"  name="type_subject_id" value="<?=$rowGen->type_subject_id?>"required>
                  <label for="exampleInputEmail1">ประเภทรายวิชา</label>
                  <input type="text" class="form-control"  style="width:100%;"  name="type_subject_name_edit" value="<?=$rowGen->type_subject_name?>" placeholder="กรอกประเภทรายวิชา" required>
                
                  <label>สถานะการใช้งาน</label>
                   <select name="type_subject_status_edit" id="type_subject_status_edit" style="width:100%;"  class="form-control">
                   <option <?php if($rowGen->type_subject_status=='1'){echo "SELECTED";} ?> value="1">เปิด</option>
                   <option <?php if($rowGen->type_subject_status=='0'){echo "SELECTED";} ?> value="0">ปิด</option>
                  </select>
