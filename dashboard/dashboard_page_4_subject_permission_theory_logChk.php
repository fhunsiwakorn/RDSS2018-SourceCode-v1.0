...
$school_id_post= $_POST['school_id_post'];
$crt_by=$_SESSION['userSession'];
$crt_date=date("Y-m-d H:i:s");
...
echo "<script>";
  ...
  $qs=time();
     echo "<script>";
    echo "location.href = '?option=permission-theory-subjetct&qs=$qs&vehicle_type=$vehicle_type_id&school=$school_id&error&$Caltime&$st_hour'";
    echo "</script>";
...
try {
  ...
  $sql_process->update($table, $fields,$Where);
   ...
   $exception->getMessage();  // Should be handled with a proper error message.
    ...
    echo "<script>";
    echo "location.href = '?option=permission-theory-subjetct&school=$school_id&error'";
    echo "</script>";
  }
}
