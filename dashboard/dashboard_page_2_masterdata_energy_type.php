...
$type_energy_list = new masterdata_general();
if(isset($_POST['btn-submit-add']))
{

  $type_energy_name = strip_tags($_POST['type_energy_name']);
  $type_energy_status = strip_tags($_POST['type_energy_status']);
  $crt_by=$_SESSION['userSession']; 
  $crt_date=date("Y-m-d H:i:s");
  $type_energy_list->add_energy_type($type_energy_name,$crt_by,$crt_date,$type_energy_status);
    echo "<script>";
    echo "location.href = '?option=Energy-type&success'";
    echo "</script>";
}
if(isset($_POST['btn-submit-edit']))
{
    $type_energy_id = strip_tags($_POST['type_energy_id']);
    $type_energy_name = strip_tags($_POST['type_energy_name_edit']);
    $type_energy_status = strip_tags($_POST['type_energy_status_edit']);
    $upd_by=$_SESSION['userSession'];
    $type_energy_list->edit_energy_type($type_energy_id,$type_energy_name,$upd_by,$type_energy_status);
    echo "<script>";
    echo "location.href = '?option=Energy-type&success'";
    echo "</script>";
} 
if(isset($_POST['DELtype_energy_id'])) {
  $count=count($_POST['DELtype_energy_id']);
  for($i=0;$i<$count;$i++){
   $type_energy_id = $_POST['DELtype_energy_id'];
   $type_energy_list->delete_energy_type($type_energy_id[$i]);
   echo "<script>";
   echo "location.href = '?option=Energy-type&success'";
   echo "</script>";
    }
...
<section class="content-header">
  ...
  <li ><a href="?option=Energy-type">ข้อมูลตั้งต้น</a></li>
  <li class="active">ประเภทเชื้อเพลิง</li>
  </ol>
</section>
<!-- Main content -->
<section class="content">
  ...
  <div class="box">
    <div class="box-header with-border">
      <h3 class="box-title">ประเภทเชื้อเพลิง</h3><br>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title">ประเภทเชื้อเพลิง</h4>
              </div>
              <form  method="post" name="form_add"id="form_add">
              <div class="modal-body">          
              <div class="form-group">
                  <label for="exampleInputEmail1">ประเภทเชื้อเพลิง</label>
                
                  <input type="text" class="form-control" name="type_energy_name" id="type_energy_name" required placeholder="กรอกประเภทเชื้อเพลิง">
                </div>    
                <div class="form-group">

                   <label>สถานะการใช้งาน</label>
                   <select name="type_energy_status" id="type_energy_status"  class="form-control">
...
<?php
$num_gen='0';
$qg = $type_energy_list->runQuery(
"SELECT
tbl_master_type_energy.type_energy_id,
tbl_master_type_energy.type_energy_name,
tbl_master_type_energy.crt_by,
tbl_master_type_energy.crt_date,
tbl_master_type_energy.upd_by,
tbl_master_type_energy.upd_date,
tbl_master_type_energy.type_energy_status,
...
tbl_user
WHERE
tbl_master_type_energy.crt_by = tbl_user.user_id AND
...
<div class="checkbox">
<label style="font-size: 1em">
<input type="checkbox"  name="DELtype_energy_id[]" id="pNUM"  value="<?=$rowGen->type_energy_id?>">
<span class="cr"><i class="cr-icon glyphicon glyphicon-ok"></i></span>
</label>
 <!-- <?php echo  sprintf("%02d", $num_gen); ?>.) -->
</div>
</td>
        <td><?=$rowGen->type_energy_name?></td>
        <td align="center" >
            <?php
            if($rowGen->type_energy_status=='1'){
              echo '<small class="label label-success"  data-toggle="tooltip" title="เปิด"><i class="fa fa-clock-o"></i> เปิด</small>';
            }elseif ($rowGen->type_energy_status=='0') {
        ...
        <div class="modal fade" id="modal-edit-<?=$num_gen?>">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <input type="hidden" class="form-control"  name="type_energy_id" value="<?=$rowGen->type_energy_id?>"required>
                  <label for="exampleInputEmail1">ประเภทเชื้อเพลิง</label>
                  <input type="text" class="form-control"  style="width:100%;"  name="type_energy_name_edit" value="<?=$rowGen->type_energy_name?>" placeholder="กรอกประเภทเชื้อเพลิง" required>
                
                  <label>สถานะการใช้งาน</label>
                   <select name="type_energy_status_edit" id="type_energy_status_edit" style="width:100%;"  class="form-control">
                   <option <?php if($rowGen->type_energy_status=='1'){echo "SELECTED";} ?> value="1">เปิด</option>
                   <option <?php if($rowGen->type_energy_status=='0'){echo "SELECTED";} ?> value="0">ปิด</option>
                  </select>
