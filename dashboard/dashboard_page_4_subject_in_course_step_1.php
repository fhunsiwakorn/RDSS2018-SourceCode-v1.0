...
try {
 ...
 $sql_process->fastQuery("UPDATE tbl_course_data SET course_success='1'   WHERE course_data_id='$course_data_id'");
    echo "<script>";
    echo "location.href = '?option=form-course-step2&manageSubject=$course_data_id&success'";
    echo "</script>";
  }
...
<section class="content">
  ...
  <div class="box">
   ...
   <label for="">  ภาคทฤษฎี <?=$dataRow["course_data_theory_hour"]?> ชั่วโมง ภาคปฎิบัติ <?=$dataRow["course_data_practice_hour"]?> ชั่วโมง รวมเป็น <?=$dataRow["course_data_total_hour"]?> ชั่วโมง  ประเภทหลักสูตร <?=$dataRow["vehicle_type_name"]?></label>
    <br><br>
    <center><h4><font color="blue">ขั้นตอนที่ 1 กำหนดรายวิชาภาคทฤษฏี (ให้ครบ <?=$dataRow["course_data_theory_hour"]?> ชั่วโมง)</font> </h4></center>
...
");
// tbl_subject_data.type_study_id = '1' ID ของภาคทฤษฎี ตาราง tbl_subject_data
// $qg->execute();
$qg->execute(array(":vehicle_type_id_param"=>$dataRow['vehicle_type_id'],":school_id_param"=>$dataRow['school_id']));
...
<?php } ?>
    ...
    <center> 
      ...
      <input type="text" name="Timeset" id="Timeset" style="width:150px; height:40px;font-size:24px;text-align: center;" value="<?=$dataRow["course_data_theory_hour"]?>" readonly >
    </center>
    <!-- <center>  <button type="submit" name="btn-submit-chk" class="btn btn-primary">คัดกรองรายวิชา</button></center> -->
    <hr>
