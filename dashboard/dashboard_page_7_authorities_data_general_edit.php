...
echo "<script>";
echo "location.href = '?option=authorities-general-edit&auttid=$user_id&success'";
echo "</script>";
  }
}
...
function dochange(src, val) {
     ...
     req.open("GET", "../library/school_and_branch.php?school_idE=<?=$dataRow['school_id']?>&branch_idE=<?=$dataRow['branch_id']?>&user_group_idE=<?=$dataRow['user_group_id']?>&data="+src+"&val="+val); //สร้าง connection
     req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=utf-8"); // set Header
     req.send(null); //ส่งค่า
}

window.onLoad=dochange('school_id', -1);
window.onLoad=dochange('branch_id', -1);
