...
//$chkprogramRows = $sql_process->rowsQuery("SELECT permission_id FROM tbl_program_permission WHERE user_group_id='$user_group_id' AND grp_id='$rowGen->grp_id' AND prog_id='$rowGen->prog_id' AND  school_id='$school_id'");
$stmt = $sql_process->runQuery("SELECT user_group_id,grp_id,prog_id FROM tbl_program_permission WHERE user_group_id=:user_group_id_param AND grp_id=:grp_id_param AND prog_id=:prog_id_param AND school_id=:school_id_param");
