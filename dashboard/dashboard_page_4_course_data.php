...
require_once('dashboard_page_4_process_subject_in_course.php');
...
<div class="checkbox">
<label style="font-size: 1em">
<input type="checkbox"  name="DELcourse_data_id[]" id="pNUM"  value="<?=$rowGen->course_data_id?>" <?php if($total_data >=1){ echo "disabled";}?>>
<span class="cr"><i class="cr-icon glyphicon glyphicon-ok"></i></span>
</label> 
<!-- <?php echo  sprintf("%02d", $num_gen); ?>.) -->
      ...
      ?>
      <button type="button" class="btn btn-default"  OnClick="move2<?=$num_gen?>()" data-toggle="tooltip" title="กำหนดวิชาเรียนตามระเบียบกรมขนส่งทางบก">
                  ...
                  <script>
                        function move2<?=$num_gen?>() {
                          ...
                          function(isConfirm){
                            ...
                            location.href = '?option=course-data&course=<?=$rowGen->course_data_id?>&group=<?=$rowGen->course_group_id?>&vehicle_type=<?=$rowGen->vehicle_type_id?>&school=<?=$rowGen->school_id?>';
                            } else {
      ...
      //  swal('กำหนดรายวิชาในหลักสูตร')
        ...
        <button type="button" class="btn btn-warning"  onclick="window.location.href='?option=course-data-edit&coecode=<?=$rowGen->course_code?>'" >
        แก้ไข
