...
if(isset($_POST['btn-submit-add']))
{
	...
	$district_list->add_district($district_code,$district_name,$amphur_id,$province_id,$crt_by,$crt_date,$district_status);
    echo "<script>";
    echo "location.href = '?option=district&success'";
    echo "</script>";
}

if(isset($_POST['DELdistrict_id'])) {
  $count=count($_POST['DELdistrict_id']);
  for($i=0;$i<$count;$i++){
   $district_id = $_POST['DELdistrict_id'];
   $district_list->delete_district($district_id[$i]);
   echo "<script>";
   echo "location.href = '?option=district&success'";
   echo "</script>";
    }
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title">เพิ่มตำบล</h4>
              </div>
              <form  method="post" name="form_add"id="form_add">
              <div class="modal-body">   
              <div class="form-group">
                  <label for="exampleInputEmail1">รหัสตำบล</label>
                  <input type="text" class="form-control" maxlength="6" name="district_code" id="district_code" required placeholder="รหัสตำบล">
                  </div>
                  <div class="form-group">
                  <label for="exampleInputEmail1">ตำบล</label>
                  <input type="text" class="form-control" name="district_name" id="district_name" required placeholder="กรอกตำบล">
             </div>      
                ...
                <div class="form-group">
                   ...
                   <select name="district_status" id="district_status"  class="form-control">
    ...
    <div class="box-body">
      ...
      <input name="h_arti_id" type="hidden" id="h_arti_id" value="" />
        <div class="input-group">
          <input type="text" name="q" id="q" class="form-control" placeholder="ค้นหาตำบล..." >
    ...
    function make_autocom(autoObj,showObj){  
        ...
        new Autocomplete(mkAutoObj, function() {  
            ...
            if ( this.value.length < 1 && this.isNotClick )   
                return ;      
            return "../library/search_data_district.php?q=" +encodeURIComponent(this.value);  
        });   
    }     
...
$total_data =$qGeneral->rowsQuery("SELECT tbl_location_district.district_id FROM tbl_location_district WHERE tbl_location_district.is_delete ='1'");
$rows='20';
if($page<=0)$page=1;
...
$qg = $district_list->runQuery(
"SELECT
tbl_location_district.district_id,
tbl_location_district.district_code,
tbl_location_district.district_name,
tbl_location_district.amphur_id,
tbl_location_district.province_id,
tbl_location_district.crt_by,
tbl_location_district.crt_date,
tbl_location_district.upd_by,
tbl_location_district.upd_date,
tbl_location_district.district_status,
...
tbl_user
WHERE
tbl_location_district.amphur_id = tbl_location_amphur.amphur_id AND
tbl_location_district.province_id = tbl_location_province.province_id AND
tbl_location_district.crt_by = tbl_user.user_id AND
tbl_location_district.is_delete ='1' AND
tbl_location_district.district_name  LIKE '%$seq%'
ORDER BY
tbl_location_district.district_name ASC limit $start,$rows
");
$qg->execute();
...
<span class="cr"><i class="cr-icon glyphicon glyphicon-ok"></i></span>
        ...
        <td>
        <div  align="center">
        <button type="button" onclick="window.location.href='?option=district-edit&edd=<?=$rowGen->district_id?>'" class="btn btn-default" data-toggle="tooltip" title="แก้ไข">
...
<?php } ?>
    ...
    <form method="get" name="supage1">
                ...
                <input type="hidden" name="option"  value="district">
                <ul class="pager">
                  <li <?php  if($page==1){echo"class='disabled'";} ?>><a href="?option=district&q=<?=$seq?>&page=<?=$page-1?>"><i class="glyphicon glyphicon-triangle-left"></i><i class="glyphicon glyphicon-triangle-left"></i></a></li>
                  ...
                  <li <?php  if($page==$total_page){echo"class='disabled'";}?>><a href="?option=district&q=<?=$seq?>&page=<?=$page+1?>"><i class="glyphicon glyphicon-triangle-right"></i><i class="glyphicon glyphicon-triangle-right"></i></a></li>
                </ul>
                </nav>
              </form>   
</div>
